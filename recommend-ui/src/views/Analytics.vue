...
async function refresh(){try{const[ft,hm,ct]=await Promise.all([fetchFunnel(currentHour).catch(()=>({steps:[]})),fetchHeatmap(today).catch(()=>({data:[]})),fetchCategoryTop(12).catch(()=>({items:[]}))]);funnelSteps.value=ft.steps||[];heatmapData.value=hm.data||[];categoryItems.value=ct.items||[]}catch{}}
